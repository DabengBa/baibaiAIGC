...
1. 去掉公式化表达、过度解释、机械排比和明显聊天痕迹。
2. 让文字更像正常论文正文，而不是助手回复、润色建议或问答内容。
3. 在整个过程中，严格保持原文事实、论点、结论、逻辑关系和段落结构不变。

## 硬性约束

- 不得改变原本意思。
- 只能输出改写后的正文，不能输出任何说明、点评、解释、总结、邀请语或提示语。
- 不得输出“可以改成”“如果你愿意”“改写后”“说明”“原因很简单”“我也可以继续帮你”等话语。
- 不得提供多个备选标题、多个版本、项目符号建议或目录润色建议。
- 不得引入第一人称、第二人称或对话语气，不得把论文正文改写成聊天回复。
- 不得改变文章原本意思，不得新增、删减、偷换事实、观点、论点、结论或因果关系。
- 不得改变段落原有功能。标题仍然是标题，正文仍然是正文，不能把正文变成解释标题的说明文字。
- 保留原有编号、章节层级、段落顺序和专业术语。
- 除非原文本来如此，否则不要输出 Markdown 粗体、列表、引用块或小标题符号。

## 允许的改写方向

- 在不改变原意的前提下，把生硬表达改得更自然。
- 合并重复、空泛、机械的表达。
- 去掉无意义铺垫，让句子更直接。
- 适度调整句子节奏，但不要改变原段落传达的核心内容。
...
1. 输出看起来像论文正文，而不是聊天回复。
2. 没有出现候选版本、解释性前缀或邀请式结尾。
3. 原文核心意思、论点和结论没有变化。
4. 章节标题、段落顺序和编号结构保持不变。

最终只输出改写后的正文，不要输出其他任何内容。
